...
import { View, SectionList, Text, Alert, Share } from "react-native";
...
class RoomActionsView extends React.Component {
  ...
  onPressTouchable = (item) => {
    ...
    if (route) {
      logEvent(
        events[
          `RA_GO_${route.replace("View", "").toUpperCase()}${
  ...
  get sections() {
    ...
    if (isGroupChat) {
      sections[2].data.unshift({
        ...
        name: I18n.t("Members"),
        description:
          membersCount > 0 ? `${membersCount} ${I18n.t("members")}` : null,
  ...
  leaveChannel = () => {
    ...
    Alert.alert(
      ...
      I18n.t("Are_you_sure_you_want_to_leave_the_room", {
        room: RocketChat.getRoomTitle(room),
      }),
      [
        {
          text: I18n.t("Cancel"),
      ...
      <>
        ...
        <View
          style={[
            ...
            item.disabled && styles.roomTitlePadding,
          ]}>
          {room.t === "d" ? (
            <Text
  ...
  renderTouchableItem = (subview, item) => {
    ...
    return (
      <Touch
        ...
        testID={item.testID}
        theme={theme}>
        <View style={styles.sectionItem}>{subview}</View>
      </Touch>
    );
  };
      ...
      item.type === "danger" ? (
        <>
          <CustomIcon
            name={item.icon}
            size={24}
            style={[styles.sectionItemIcon, colorDanger]}
          />
        ...
        <>
          <CustomIcon
            name={item.icon}
            size={24}
            style={[styles.sectionItemIcon, { color: themes[theme].bodyText }]}
          />
          <Text
            style={[styles.sectionItemName, { color: themes[theme].bodyText }]}>
            {item.name}
          </Text>
          {item.description ? (
            <Text
